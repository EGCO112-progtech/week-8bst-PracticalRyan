      ...
      else
      {
         while (!inserted)
         {
            if (t->data >= value)
            {
               ...
               if (!t->leftPtr) // If right doesn't exist, set new node as next node
               {
                  t->leftPtr = new_node;
                  inserted = 1;
               }
               else // If right does exist, set t to next node
               {
                  t = t->leftPtr;
               }
            }
            else
            {
               ...
               if (!t->rightPtr) // If right doesn't exist, set new node
               {
                  t->rightPtr = new_node;
                  inserted = 1;
               }
